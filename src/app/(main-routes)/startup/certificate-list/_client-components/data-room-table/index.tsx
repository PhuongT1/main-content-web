...
const DataRoomTable = () => {
	...
	const listTypeQuery = (
		searchParams.get('listType') as SORT_TYPE
	) || SORT_TYPE.POPULARY
	const pageQuery = Number(searchParams.get('page')) || 1
	const typeQuery = (
		searchParams.get('type') as CONTENT_TYPE
	) || null
	...
	return (
		<Stack direction={'column'} gap={{ md: 6, sm: 3 }}>
			{referenceCate && referenceCate.data && !cateFetching ? (
				<FilterBar sort={listType} seletedCate={categoryValue} cate={referenceCate?.data}
				           keyword={keyword} />
			) : null}
