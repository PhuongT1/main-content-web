...
import { SloganAtom } from '../swot-atom'
...
function HocStepSwot<T, T2>(WrappedComponent: ComponentType<propStepComponent<T, T2>>, variable: PropvVariable) {
  const EnhancedComponent = ({ ...props }: {projectId: number}) => {
    ...
    const { data, isFetching: isFetchingCurrent, isLoading: isLoadingCurrent } = useQuery({
      queryKey: [
        'get-data-swot',
        {
    ...
    const { data: dataPrewStep, isFetching: isFetchingPrev, isLoading: isLoadingPrev } = useQuery({
      queryKey: [
        'get-data-swot-prev',
        {
    ...
    return (
      <>
        <WrappedComponent
          ...
          onFinish={onFinish}
          showEdit={showEdit}
          isLoadingCurrent={isLoadingCurrent || isFetchingCurrent}
          isLoadingPrev={isLoadingPrev || isFetchingPrev}
        />
