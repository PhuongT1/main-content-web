			...
			<DialogContent className={`${styles.popup_wrapper} ${type === 'dark' ? styles.dark : styles.light}`}>
				<div className={`${styles.content_wrapper}`}>
					...
					<Box component={'form'} sx={{ width: '100%' }} onSubmit={handleSubmit(onSubmit)}>
						<Box display={'flex'} flexDirection="column" alignItems="flex-start" width={'100%'} my={5} gap={2.5}>
							<Box display={'flex'} flexDirection="column" alignItems="flex-start" width={'100%'}>
								...
								>
									<Avatar
										...
										src={!!startupData?.user?.avatar?.url ? startupData?.user?.avatar?.url : '/images/blank-user.png'}
									/>
									<Typography
										cate="body_30"
										color={type === 'dark' ? theme.palette.main_grey.gray100 : theme.palette.main_grey.gray950}
										ml={1.5}
										flexShrink={(
											startupData?.user?.nickname?.length || 0
										) > 6 ? 1 : 0}
										sx={
											(
												startupData?.user?.nickname?.length || 0
									...
									{/* <Typography
									 ...
									 >
									 {`(${startupData?.experiences[0]?.undertaking} ${startupData?.experiences[0]?.yearsOfExperience}년차, @${startupData?.projects[0]?.projectOwner} ${startupData?.projects[0]?.projectName})`}
									 </Typography> */}
								</Box>
							</Box>
							<Box display={'flex'} flexDirection="column" alignItems="flex-start" width={'100%'}>
